#!/usr/bin/env bash
# Publishes velyr-real-demo.mp4 as an Instagram Reel (@velyr.io).
...
echo "== staging video (litterbox, fallback uguu.se) =="
VURL=$(curl -s --max-time 90 -F reqtype=fileupload -F time=1h -F fileToUpload=@"$VIDEO" https://litterbox.catbox.moe/resources/internals/api.php || true)
if ! echo "$VURL" | grep -q '^https'; then
  echo "litterbox failed ($VURL), trying uguu.se (24h expiry)"
  VURL=$(curl -s --max-time 90 -F "files[]=@$VIDEO" "https://uguu.se/upload?output=text" || true)
fi
echo "video -> $VURL"
echo "$VURL" | grep -q '^https' || { echo "staging failed: $VURL"; exit 1; }
# sanity: the staged URL must serve raw mp4, not an HTML page (tmpfiles gotcha)
curl -s --max-time 30 -r 0-15 "$VURL" | grep -q ftyp || { echo "staged URL does not serve raw mp4"; exit 1; }
