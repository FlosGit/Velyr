...
| `AGENT_FIX_VERIFY` | edge `index.ts` (`applyVerifyGate`) | Supabase only (no Vercel twin — the gate lives entirely edge-side) | on — `(env ?? 'true') !== 'false'` | Verify-gate (2026-07-14): adversarial second vision call after Pass 2, before any write, on all three paths. Refute ⇒ honest `skipped_low_confidence` with `analysis_result.verify` + `refuted_fix`; verifier errors fail OPEN. Trip-wire: refute-rate >50% over ≥4 gated runs ⇒ set `false` + autopsy |
| `AGENT_HTML_EDIT` | edge `deep-reader.ts` (promotion) + `index.ts` (createPR `.html` allowlist + W4 sparseOk gate) — THREE read sites, keep together | Supabase only | on — `(env ?? 'true') !== 'false'` | W3 (2026-07-14): root `index.html` becomes an editable component on `vite-react`/`plain-html` runs, validated by `validateHtmlEdit` (comparative; `html-validate.ts`). `false` restores read-only head-extract context AND disables the W4 sparse-shell path |
| `AGENT_SPARSE_SHELL_FIX` | edge `index.ts` (plain-GitHub ranker call site, `sparseShellOk`) | Supabase only | on — `(env ?? 'true') !== 'false'` | W4 (2026-07-15): own kill-switch for the plain-html sparse-shell path (below-gate graph included without LLM ranking → W3 editable shell). Only effective while `AGENT_HTML_EDIT` is also on |
| `AGENT_LLM_TIMEOUT_MS` / `AGENT_LLM_TIMEOUT_IMAGES_MS` | edge `index.ts` (`LLM_TIMEOUT_MS` / `LLM_TIMEOUT_IMAGES_MS`, used in `callLLMCapped`) | Supabase only | 120000 / 160000 | LLM call wall-clock timeouts, Opus-tuned 2026-07-15 (were hardcoded 45s/90s Sonnet-era). Timeouts on calls with a budget ≥60s are NOT retried in `postOpenRouterWithRetry` (wall-clock protection); re-tune when switching model speed classes |
...
| `AGENT_MIN_GRAPH_NODES` | `component-ranker.ts:55` | Supabase | 3 | Sparse-graph skip gate (plain-html is exempt since W4 2026-07-15 via `sparseOk`; all other frameworks below the gate skip) |
| `AGENT_APP_ROUTER_MAX_ENTRIES` | `repo-mapper.ts:283` | Supabase | 25 | App Router entry-point discovery cap |
| `LLM_MAX_TOKENS_ANALYSIS` / `LLM_MAX_TOKENS_RANKER` / `LLM_MAX_TOKENS_ROAST` | edge `index.ts:59,68,60` | Supabase | 8000 / 3000 / 1500 | max_tokens per call type (analysis/ranker raised 2026-07-15 for Opus verbosity; a `finish_reason=length` now self-heals once via a doubled-cap retry in `callLLMCapped` before throwing) |
