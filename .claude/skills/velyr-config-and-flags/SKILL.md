...
| `AGENT_HTML_EDIT` | edge `deep-reader.ts` (promotion) + `index.ts` (createPR `.html` allowlist) — TWO read sites, keep together | Supabase only | on — `(env ?? 'true') !== 'false'` | W3 (2026-07-14): root `index.html` becomes an editable component on `vite-react`/`plain-html` runs, validated by `validateHtmlEdit` (comparative; `html-validate.ts`). `false` restores read-only head-extract context |
