...
| Q4 | Why are runs skipping? | Occasional, varied | A dominant skip status is actionable: `skipped_no_data` → PostHog partition/traffic; `skipped_insufficient_graph` → sparse-gate (MIN_GRAPH_NODES=3; plain-html no longer trips it since W4 2026-07-15 — a plain-html row here means a kill-switch is off); `skipped_cost_cap` → wallet cap hit; `skipped_setup_pending` → Setup-PR unanswered. |
