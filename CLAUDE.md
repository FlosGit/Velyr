...
- `agent_business_dna` — persistent outcome log (`pending` → `success` after 7d, or `rollback`); `user_verdict` (`'confirmed'`/`'rejected'`/NULL, dashboard DNA tab) — rejected entries are excluded from the agent's prompt context.
