...
Auth: cron requests must carry either Vercel's `x-vercel-cron` header or `x-cron-secret: $AGENT_CRON_SECRET`. The same endpoint handles user `?action=pause|resume|delete|update-settings|reenable_snippet|trigger_run|dna_verdict|approve_run|reject_run` calls authenticated via Bearer token (Supabase user JWT), plus the un-authenticated public surfaces `?action=public-timeline` and the C12 SVG embeds `?action=win_badge|win_card&slug=<public_slug>` (both gated on `is_public`; SVG builders in `api/_lib/win-card.js`, unit-tested). `approve_run`/`reject_run` (C2) are the dashboard twins of the Telegram YES/NO — same reconcile/CAS helpers, all four run kinds (GitHub fix PR, GitHub rollback proposal, `shopify_awaiting_approval`, `shopify_rollback_pending`). Two dashboard feedback loops ride on these actions (migration `20260703_focus_page_dna_verdict.sql`): **"Fix in next run"** (Funnel tab) writes `agent_subscriptions.focus_page_path` via `update-settings`; the edge fn biases the Pass-1 ranker context + Pass-2 prompt toward the pinned page on all three pipeline paths (`loadFocusPage`), then clears the pin after Pass 2 (`clearFocusPage` — consumed even on a skip). **DNA confirm/reject** (`dna_verdict`) stamps `agent_business_dna.user_verdict`; `loadBusinessDNA` excludes `'rejected'` entries from the prompt and labels `'confirmed'` ones owner-confirmed.
...
- Inline buttons (C1): approval messages carry ✅/❌ buttons (`callback_data: approve:<runId>` / `reject:<runId>`) plus, where a preview exists, a `🔍 Preview` button (`preview:<runId>`): on a GitHub fix PR it resolves the PR's CI preview deployment (GitHub Deployments API) and sends a before/after screenshot pair + link (C4); on a Shopify-direct approval — only when `AGENT_SHOPIFY_PREVIEW_THEMES=1` on both surfaces — it stages the fix onto a throwaway duplicate theme and sends the `?preview_theme_id` link (C3; duplicate auto-deleted on decide)
- `status` — last 5 runs + tracked competitors
- `dna` — view Business DNA learnings
- `note <reason>` — attaches to the last rejected OR `skipped_low_confidence` run; on a rejected run it's stored `outcome:'negative'` (never-do-again signal), on a skipped run `outcome:'neutral'` + `metric_type:'manual'` = **owner context** (the C11 question-answer loop) — injected into future prompts as OWNER CONTEXT, and it counts as signal for the no-data gate. Explicit `note <run-id> <reason>` still works as an unadvertised fallback.
...
- `impact_metrics` — matched-window (deploy±2d) bounce rate before/after per run; `metric_type` is `site_wide_bounce_rate` or `route_scoped_bounce_rate` (plus legacy `bounce_rate`), and — when `conversion_goal_event` is set — an additional `goal_conversion_rate` row (sessions with ≥1 goal event / all sessions; measurement only, never a rollback trigger). The dashboard Runs chip, public timeline, and DNA promotion all read this — never render `agent_runs.bounce_rate_before/after` as a pair (mixed windows).
...
- **Engagement enrichment (edge-only):** the edge `getPostHogAnalytics` additionally returns `last7Days.engagement` — per-page scroll depth (avg max-scroll % from `$pageleave` `$prev_pageview_max_scroll_percentage`, so bounced single-page sessions count) and the top clicked elements (`$autocapture` `$el_text`), both **device-split since item 3b** (grouped by `$device_type`, merged client-side: per-page `byDevice.Mobile/Desktop` scroll + per-element `mobileShare`; a null `mobileShare` means the rows lacked device data — render as unknown, never 0%). Two frustration signals ride alongside, each with its own isolated catch and the same per-page + `mobileShare` shape: **rage-clicks** (`$rageclick`, C6 — posthog-js default) and **dead clicks** (`$dead_click` — needs PostHog's project-level "dead clicks autocapture" toggle, enabled 2026-07-07; zero rows render as n/a). It's the "heatmap" signal that lets the Pass-2 fix prompt ground a hypothesis in real behavior ("the CTA is below the fold and most visitors never reach it") instead of inferring from code layout. The scroll/click/rage queries rely on posthog-js default autocapture, so **no customer snippet change is needed**. The block is gated on real traffic (`MIN_UNIQUE_VISITORS_7D`) and wrapped in its own try/catch so it can never null-out the core analytics. It is **deliberately not mirrored** in the `api/agent/run.js` twin (that twin powers reporting modes that don't feed the LLM); only the `$host` filter is twin-locked.
...
- `SCREENSHOTONE_API_KEY` — screenshot capture for rollback comparison + the C4 preview button (optional; both degrade gracefully if absent). The capture pipeline lives in `api/_lib/screenshot.js` (shared by `run.js` + the Telegram webhook).
- `AGENT_SHOPIFY_PREVIEW_THEMES` — C3 preview themes. `=1` on BOTH Vercel (webhook executes) and the edge fn (shows the button). **Dev-store-verified + enabled 2026-07-07** (`scripts/shopify-dv-verify.mjs`, all 7 checks incl. the async-`themeDuplicate` processing wait). `themeDuplicate`/`themeDelete` are Admin API **2026-07** shapes — the theme-level helpers pin their own version, separate from the file-level 2026-04 pin; a fresh duplicate stays `processing: true` while Shopify copies files, so every write/delete against it must go through `waitForThemeReady` first.
