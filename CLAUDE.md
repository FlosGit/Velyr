...
- `AGENT_SHOPIFY_PREVIEW_THEMES` — C3 preview themes, **default OFF**. `=1` on BOTH Vercel (webhook executes) and the edge fn (shows the button). Enable only after `scripts/shopify-dv-verify.mjs` steps (5)+(6) pass on a dev store (`themeDuplicate`/`themeDelete` are Admin API **2026-07** shapes — the theme-level helpers pin their own version, separate from the file-level 2026-04 pin).
