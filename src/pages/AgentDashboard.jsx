...
// Stylesheets, images, and other non-source assets are real import-graph nodes
// but not "surfaces" a customer reasons about — they're dropped from the graph
// (their dangling edges auto-filter in SiteNetwork's settle() validId check).
const NETWORK_ASSET_RE = /\.(css|scss|sass|less|styl|svg|png|jpe?g|webp|gif|ico|woff2?|ttf|eot)$/i

// Map an RA2 file path to a SiteNetwork cluster, or null to drop the node.
// Priority: assets dropped → directory segments → filename keywords →
// parent-dir fallback (an app's unrecognised components are product, not
// marketing — validated against the FlosGit/Test fitness repo's 45 nodes).
function clusterFromPath(rawPath) {
  ...
  // Directory segments (walk from innermost outward)
  ...
  if (/^use[a-z]/.test(f))                                                    return 'utility'  // useXxx not in /hooks/
  ...
  // data/config modules → utility (real deps, not surfaces)
  ...
  if (seg.some(s => ['components','ui','shared','common'].includes(s))) {
    ...
    return 'product'  // an app's components are product features by default
  }
...
function labelFromNode(n) {
  // componentName is the default-export name from RA2 — best label if present.
...
function NetworkPage({ runs, siteNetwork, websiteUrl }) {
  ...
  const networkData = siteNetwork?.nodes?.length > 0 ? (() => {
    ...
    const enrichedNodes = siteNetwork.nodes
      .map(n => {
        const cluster = clusterFromPath(n.id)
        if (cluster === null) return null   // asset (CSS/image/font) — not a surface
        // tracked for background/infra clusters; neutral for everything else
        let status      = (cluster === 'legal' || cluster === 'utility') ? 'tracked' : 'neutral'
        let statusSource = null
        // fix-in-flight: running or waiting_approval run is editing this exact file
        ...
        return {
          ...
          route:        null,   // route derivation deferred; file path shown via tooltip
          cluster,
          status,
          statusSource,
          isEntry:      n.depth === 0,
    ...
    // Real import edges, kept only when BOTH endpoints survived the asset drop.
    const survivingIds = new Set(enrichedNodes.map(n => n.id))
    const importEdges = (siteNetwork.edges || [])
    ...
    return {
      meta: {
        ...
        totalEdges:     importEdges.length + hubEdges.length,
      },
      nodes: [hubNode, ...enrichedNodes],
      edges: [...importEdges, ...hubEdges],
    }
  })() : null
